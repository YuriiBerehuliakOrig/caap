/// Mutable IR graph storage, mirroring Python's `IRGraph`.
...
pub struct IRGraph {
    ...
    source_spans: BTreeMap<NodeId, SourceSpan>,
...
impl IRGraph {
    ...
    pub fn set_node(&mut self, node: Node, parent_id: Option<NodeId>) {
        let id = node.id();
    ...
    pub fn delete_node(&mut self, node_id: NodeId) -> CaapResult<bool> {
        ...
        self.delete_node_unchecked(node_id);
        Ok(true)
    }

    pub fn delete_node_unchecked(&mut self, node_id: NodeId) -> bool {
    ...
    pub fn drop_subtree_unchecked(&mut self, root_id: NodeId) -> Vec<NodeId> {
        ...
        while let Some(current) = stack.pop() {
            ...
            if self.delete_node_unchecked(current) {
    ...
    pub fn erase_detached_subtree(&mut self, root_id: NodeId) -> CaapResult<Vec<NodeId>> {
        ...
        Ok(self.drop_subtree_unchecked(root_id))
    }
    ...
    pub fn replace_subtree(&mut self, old_id: NodeId, new_id: NodeId) -> CaapResult<Vec<NodeId>> {
        ...
        if self.has_top_level_form(old_id) {
            ...
            return Ok(self.drop_subtree_unchecked(old_id));
        }
        ...
        Ok(self.drop_subtree_unchecked(old_id))
    }

    pub fn insert_expr_spec(&mut self, spec: &ExprSpec) -> CaapResult<NodeId> {
        self.insert_expr_spec_with_parent(spec, None)
    }

    fn insert_expr_spec_with_parent(
        &mut self,
        spec: &ExprSpec,
        ...
        let id = match spec {
            ...
            ExprSpec::Literal(literal) => {
                let id = self.allocate_id();
                self.set_node(
                    crate::ir::Node::Literal(crate::ir::LiteralNode::new(
                        id,
                        literal.value.clone(),
                    )),
                    parent_id,
            ...
            ExprSpec::Call(call) => {
                let id = self.allocate_id();
                let callee = self.insert_expr_spec_with_parent(&call.callee, Some(id))?;
                let mut args = Vec::with_capacity(call.args.len());
                for arg in &call.args {
                    args.push(self.insert_expr_spec_with_parent(arg, Some(id))?);
                }
                self.validate_call_children(callee, &args)?;
                self.set_node(
        ...
        if let Some(span) = spec.span() {
            self.set_source_span(id, span.clone())?;
        }
        Ok(id)
    }
    ...
    pub fn to_template(&self) -> IRGraphTemplate {
        ...
        let parents: Vec<(NodeId, Option<NodeId>)> = self
            .parents
            .iter()
            .map(|(&id, &parent)| (id, parent))
            .collect();
        let source_spans: Vec<(NodeId, SourceSpan)> = self
            .source_spans
            .iter()
    ...
    pub fn validate(&self) -> CaapResult<()> {
        ...
        for &id in &ids {
            ...
            while let Some(current_id) = current {
                ...
                current = parent_by_id.get(&current_id).copied().flatten();
            }
        }
...
impl GraphBuilder {
    ...
    pub fn name(&mut self, identifier: impl Into<String>) -> NodeId {
        self.try_name(identifier)
            .expect("GraphBuilder::name requires a non-empty identifier")
    }
    ...
    pub fn literal(&mut self, value: crate::ir::IrLiteralData) -> NodeId {
        self.try_literal(value)
            .expect("GraphBuilder::literal received invalid literal data")
    }
    ...
    pub fn call(&mut self, callee: NodeId, args: Vec<NodeId>) -> NodeId {
        self.try_call(callee, args)
            .expect("GraphBuilder::call requires existing callee and argument nodes")
    }
    ...
    pub fn lower_spec(&mut self, spec: &ExprSpec) -> CaapResult<NodeId> {
        self.lower_spec_with_parent(spec, None)
    }

    fn lower_spec_with_parent(
        &mut self,
        spec: &ExprSpec,
        parent_id: Option<NodeId>,
    ) -> CaapResult<NodeId> {
        let id = match spec {
            ...
            ExprSpec::Call(call) => {
                let id = self.graph.allocate_id();
                let callee = self.lower_spec_with_parent(&call.callee, Some(id))?;
                let mut args = Vec::with_capacity(call.args.len());
                for arg in &call.args {
                    args.push(self.lower_spec_with_parent(arg, Some(id))?);
                }
                self.graph.validate_call_children(callee, &args)?;
                self.graph.set_node(
                    crate::ir::Node::Call(crate::ir::CallNode::new(id, callee, args)),
        ...
        if let Some(span) = spec.span() {
            self.graph.set_source_span(id, span.clone())?;
        }
        Ok(id)
    }
}
